...
import eu.kanade.tachiyomi.extension.all.booklore.dto.AuthorDto
import eu.kanade.tachiyomi.extension.all.booklore.dto.BookDto
import eu.kanade.tachiyomi.extension.all.booklore.dto.CollectionDto
import eu.kanade.tachiyomi.extension.all.booklore.dto.LibraryDto
import eu.kanade.tachiyomi.extension.all.booklore.dto.PageDto
import eu.kanade.tachiyomi.extension.all.booklore.dto.PageWrapperDto
import eu.kanade.tachiyomi.extension.all.booklore.dto.ReadListDto
import eu.kanade.tachiyomi.extension.all.booklore.dto.SeriesDto
...
open class Booklore(private val suffix: String = "") : ConfigurableSource, UnmeteredSource, HttpSource() {
    ...
    override val id by lazy {
        val key = "booklore${if (suffix.isNotBlank()) " ($suffix)" else ""}/en/$versionId"
    ...
    override fun chapterListParse(response: Response, hasNextPage: MutableList<Boolean>?): List<SChapter> {
        if (response.isFromBook()) {
            ...
            return listOf(
                SChapter.create().apply {
                    ...
                    tags = book.metadata.tags.joinToString()
        ...
        return page
            ...
            .mapIndexed { index, book ->
                SChapter.create().apply {
                    chapter_number = book.metadata.numberSort ?: -1F
                    url = "$baseUrl/api/v1/books/${book.id}"
                    description = book.metadata.summary
                    tags = book.metadata.tags.joinToString()
                    thumbnail_url = "$baseUrl/api/v1/books/${book.id}/thumbnail"
    ...
    override fun setupPreferenceScreen(screen: PreferenceScreen) {
        ...
        if (suffix.isEmpty()) {
            ListPreference(screen.context).apply {
                ...
                summary = "Number of additional sources to create. There will always be at least one Booklore source."
...
private const val PREF_CHAPTER_NAME_TEMPLATE_DEFAULT = "{title}"
